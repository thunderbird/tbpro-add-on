 ...
 * the static-key security model) and emits exactly one structured audit line
 * per request. Storage *values* are never logged, only who called, whose
 ...
 *       Thunderbird Accounts backend (issue #1216). Requires the static
 *       integration key issued by Send, presented as a Bearer token and matched
 *       against the INTERNAL_API_KEYS allowlist. The user is identified by
 *       OIDC subject; legacy users without an OIDC subject are out of scope and
 *       answer 404.
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *         schema:
 *           type: string
 *         description: OIDC subject (sub) of the user
 *     responses:
 *       200:
 ...
 *       404:
 *         description: No user with this OIDC subject
 *       503:
 *         description: No integration key configured (fail closed)
 */
router.get(
  '/users/:sub/storage',
  requireServiceAuth(),
  wrapAsyncHandler(async (req, res) => {
    ...
    const caller =
      (req as RequestWithServiceCaller).serviceCaller?.label ?? 'unknown';
    ...
    try {
      ...
      // uploads. The user-facing getTotalUsedStorage itself is untouched.
      ...
      // case is the one the audit trail most needs. The error is rethrown so
      // wrapAsyncHandler -> the global error handler produces the usual 500.
      finish(500);
      throw err;
    }
  })
