...
// raw bytes from JS. Resolves on transaction commit (not just request success)
// so the key is durably stored before anything is encrypted under it.
...
// Returns the persisted AES-GCM key if one exists, WITHOUT creating one.
//
// Use this on the decrypt path: if there is ciphertext in storage but no key in
// IndexedDB, the ciphertext is permanently unreadable — generating a fresh key
// (as getOrCreatePassphraseKey would) cannot help and would only mask the
// situation. Callers should treat a null return as "the stored payload can
// never be decrypted" and self-heal (discard the stale payload).
//
// Throws on infrastructure errors (e.g. IndexedDB failed to open) so callers
// can distinguish "no key exists" from "could not check".
export async function getExistingPassphraseKey(): Promise<CryptoKey | null> {
  if (!isCryptoStorageAvailable()) return null;
  // Reuse an in-flight/settled load (which may have created the key already).
  if (cachedKeyPromise) return cachedKeyPromise;

  const db = await openDb();
  try {
    const key = await getKeyFromDb(db);
    if (key) {
      // Seed the shared cache so later encrypt/decrypt calls skip IndexedDB.
      cachedKeyPromise = Promise.resolve(key);
    }
    // Never cache the null result: a later getOrCreatePassphraseKey() must
    // still be able to create the key.
    return key;
  } finally {
    db.close();
  }
