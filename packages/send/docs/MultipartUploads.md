...
storage (Backblaze B2 by default; also any S3-compatible bucket). A single
...
Selected by `STORAGE_BACKEND` (`backend/src/storage/index.ts`): `b2` or `s3`.
There is no filesystem option, and an unrecognised value throws at boot: uploads
and downloads are **presigned URLs** generated by an AWS-SDK S3 client
(`storage/s3b2.ts`), so a deployment without a bucket cannot serve a single file.
The bytes never transit the backend. Local development points `s3` at the MinIO
service in `compose.yml`, which needs no credentials.
