...
> Every operation — including `FileStore.length()`'s `HeadObject` — goes
> through the API that wrote the object, which is read-after-write consistent,
> so the create-entry size check and the `/stat` poll see an upload
> immediately. See
