...
> B2 and S3 run on one code path: B2's S3-compatible API takes the same
> commands, so `storage/s3b2.ts` serves both. Every operation — including
> `FileStore.length()`'s `HeadObject` — goes through the API that wrote the
> object, which is read-after-write consistent, so the create-entry size check
> and the `/stat` poll see an upload immediately. See
