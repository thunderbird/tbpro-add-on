...
def get_webhook_auth(request: Request, fxa_client: FxaClient = Depends(get_fxa_client)):
    ...
    if not auth_header:
        logger.error('FXA webhook event with no authorization.')
        return None

    header_type, header_token = auth_header.split(' ')
    if header_type != 'Bearer':
        logger.error(f'Error decoding token. Type == {header_type}, which is not Bearer!')
        return None
    ...
    if 'kid' not in headers:
        logger.error('Error decoding token. Key ID is missing from headers.')
        return None
    ...
    if jwk_pem is None:
        logger.error(f"Error decoding token. Key ID ({headers.get('kid')}) is missing from public list.")
        return None
    ...
    if decoded_jwt.get('iss') != fxa_client.config.issuer:
        logger.error(f"Issuer is not valid: ({decoded_jwt.get('iss')}) vs ({fxa_client.config.issuer})")
